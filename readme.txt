...
1. Exemplu de afișare "Una peste alta" (pe coloană) / "Stacked" column layout example.
2. Exemplu de afișare "Una lângă alta" (pe aceeași linie) / "Side by side" layout example.
...
= 1.4.9 =
* Design: Actualizat pictograma oficială a plugin-ului cu cercul SAL decupat cu fundal transparent din imaginea Codester Preview, fiind optimizat excelent atât pentru light mode, cât și pentru dark mode / Design: Updated the official plugin icon with the circular SAL badge cropped with a transparent background from the Codester Preview image, beautifully optimized for both light and dark modes.

= 1.4.8 =
* Fix: Adăugat protecție suplimentară împotriva accesului direct la fișiere în `uninstall.php` pentru respectarea riguroasă a standardelor de securitate / Fix: Added direct file access protection check in `uninstall.php` to strictly comply with security scanner standards.
...
= 1.4.3 =
* Design: Imaginile SAL și SOL au fost înlocuite cu formatul vectorial (.svg) pentru o claritate perfectă pe orice ecran, cu fallback automat la .png în caz de eroare / Design: Replaced SAL and SOL images with vector format (.svg) for perfect clarity on all screens, with automatic .png fallback.
...
= 1.4.0 =
* Branding: Adăugat Banner și Pictogramă oficială pentru pagina din WordPress.org / Added official Banner and Icon for the WordPress.org repository.

= 1.3.7 =
* Fix: Corectare mapare capturi de ecran / Fix: Corrected screenshot mapping.

= 1.3.6 =
* Fix: Corectare finală a ordinii capturilor de ecran / Fix: Final correction of screenshot order.
